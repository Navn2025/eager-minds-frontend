...
export default function Dashboard() {
  ...
  const worksheetCount = data?.completions.filter((c) => c.itemType === "worksheet").length || 0;
  const paperCount = data?.completions.filter((c) => c.itemType === "paper").length || 0;
  ...
  return (
    <div className="space-y-8 md:space-y-12 pb-32">
      ...
      >
        <div className="space-y-4">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-8xl font-black tracking-tighter text-white leading-[0.9]">
            Welcome back, <span className="text-white/20">{authUser?.name?.split(" ")[0] || "Explorer"}.</span>
          </h1>
          <p className="text-text-secondary text-base md:text-xl font-medium max-w-xl leading-relaxed">
            Your elite learning journey continues. Here is an overview of your academic mastery.
          </p>
        </div>
        <Button className="h-14 md:h-16 px-8 md:px-10 rounded-2xl md:rounded-3xl bg-accent text-white font-black uppercase tracking-widest hover:scale-[1.05] transition-all accent-glow group">
           Explore Courses <ArrowUpRight size={18} className="ml-2 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
        </Button>
      </motion.header>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {[
          { label: "Worksheets", value: worksheetCount, icon: CheckCircle2, status: "completed", color: "accent" },
          { label: "Mock Papers", value: paperCount, icon: Zap, status: "synchronized", color: "blue" },
          { label: "Study Time", value: "12h", icon: Clock, status: "this week", color: "violet" },
          { label: "Avg. Score", value: "84%", icon: TrendingUp, status: "top 5%", color: "emerald" }
        ].map((stat, i) => (
          <motion.div
            key={stat.label}
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            <div className="matte-card p-6 md:p-10 h-full group hover:bg-white/[0.02] transition-all duration-500 rounded-[2rem]">
              <div className={cn(
                ...
                stat.color === "accent" ? "bg-accent/20 text-accent border border-accent/20" : "bg-white/5 text-white/40 border border-white/5"
              )}>
                <stat.icon size={20} strokeWidth={2.5} className="md:w-7 md:h-7" />
              </div>
              <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-10">
        ...
        <div className="lg:col-span-2 matte-card p-6 md:p-10 overflow-hidden rounded-[2rem]">
          <div className="flex items-center justify-between mb-8 md:mb-12">
            <h3 className="text-xl md:text-3xl font-black uppercase tracking-tighter text-white">Subject Mastery</h3>
              ...
              data.progress.map((p, i) => (
                  ...
                  <div className="flex justify-between items-end">
                    <div className="space-y-2">
                      <div className="flex items-center gap-3">
                        ...
                        <h4 className="text-lg md:text-xl font-bold text-white tracking-tight">{p.subjectName}</h4>
                      </div>
                      <p className="text-[10px] text-white/20 font-black uppercase tracking-[0.3em] ml-5">{p.completed} of {p.total} Modules Mastered</p>
                    </div>
                    <div className="text-right">
                      <span className="text-3xl md:text-4xl font-black text-white tracking-tighter">{p.percentage}%</span>
                    </div>
                  </div>
                ...
                <div className="py-12 md:py-24 text-center flex flex-col items-center gap-6 md:gap-8">
                  ...
                  <div className="space-y-3">
                    <p className="text-[10px] md:text-[12px] text-white/40 font-black uppercase tracking-[0.3em]">Initialize Training</p>
                    <p className="text-base md:text-lg text-white/20 font-medium max-w-sm px-4">Complete your first worksheet to synchronize progress metrics.</p>
                  </div>
                </div>
            )}
          </div>
        ...
        <div className="matte-card p-6 md:p-10 overflow-hidden flex flex-col rounded-[2rem]">
          <div className="flex items-center justify-between mb-8 md:mb-12">
            <h3 className="text-xl md:text-3xl font-black uppercase tracking-tighter text-white">Grid Stream</h3>
            <div className="p-2 md:p-3 rounded-xl md:rounded-2xl bg-white/5 border border-white/10 text-white/40 shrink-0">
          ...
          <div className="flex-grow space-y-4">
              ...
              >
                ...
                <div className="relative z-10 min-w-0">
                  ...
                  <p className="text-[9px] text-white/20 font-black uppercase tracking-[0.3em] mt-1">
                    {new Date(completion.completedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                  </p>
                </div>
            ...
            {(!data?.completions || data.completions.length === 0) && (
              <div className="py-12 md:py-24 text-center flex flex-col items-center gap-6">
                ...
                <p className="text-[10px] text-white/20 font-black uppercase tracking-[0.3em] leading-relaxed">System awaiting initial data sync.</p>
              </div>
            )}
          </div>

          <div className="mt-8 md:mt-12">
             <Button variant="ghost" className="w-full h-14 md:h-16 rounded-[1.5rem] md:rounded-[2rem] border border-white/5 text-[10px] font-black uppercase tracking-[0.3em] text-white/40 hover:text-white hover:bg-white/5 transition-all">
               Full Node Analytics
            </Button>
          </div>
        </div>
