...
const normalizePdfUrl = (value?: string): string => {
  if (!value) return "";
...
export default function DashboardMagazines() {
      ...
      .then((res) => {
        ...
        const mapped = raw.map(
          (m: {
            ...
            category: m.category || "Educational",
            pageCount: m.pageCount || 42,
            pdfUrl: normalizePdfUrl(m.pdfUrl || m.fileUrl),
            isNew: true,
          }),
        );
  ...
  const openMagazine = (magazine: Magazine) => {
    const pdfUrl = normalizePdfUrl(magazine.pdfUrl || magazine.fileUrl);
    if (!pdfUrl) {
      alert("This magazine is not available to read yet.");
      return;
    }
                    ...
                    <Button
                      onClick={() => {
                        const pdfUrl = normalizePdfUrl(
                          magazines[0].pdfUrl || magazines[0].fileUrl,
                        );
                        if (!pdfUrl) {
                          alert(
                            "This magazine is not available to download yet.",
                          );
                          return;
                        }
