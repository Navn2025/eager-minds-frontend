...
export default function WordOfTheDayPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        setWordData({
          ...
          exampleSentence: "Her perseverance in learning new skills every day made her stand out.",
        });
      }
      try {
  ...
  const synonyms = wordData?.synonym
    ? wordData.synonym.split(",").map((s) => s.trim()).filter(Boolean)
    : [];
  const antonyms = wordData?.antonym
    ? wordData.antonym.split(",").map((s) => s.trim()).filter(Boolean)
    : [];
    ...
    <div className="min-h-screen bg-[#0b0b0b] pt-28 pb-20">
      <div className="container mx-auto px-6 max-w-5xl">
        ...
        >
          ...
          <p className="text-white/50 text-lg">Expand your vocabulary every day with our highlighted words.</p>
        </motion.div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {pastWords.map((pw, i) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-white/50 leading-relaxed">{pw.meaning}</p>
                    {pw.createdAt && (
                      <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.2em] text-white/20">
                        {new Date(pw.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}
                      </p>
                    )}
                  </motion.div>
                ))}
