...
export default function PapersOnDemand() {
  const { user } = useAuth();
  const isRolePremium = user?.role === "premium" || user?.role === "admin";
  ...
  const [freePaperUnlocks, setFreePaperUnlocks] = useState(0);

  const hasPaperAccess = isRolePremium || freePaperUnlocks > 0;
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        setQuizSummary(res.data);
        setFreePaperUnlocks(res.data.freePaperUnlocks || 0);
      })
      .catch(() => {
        setQuizSummary(null);
      });
  ...
  useEffect(() => {
      ...
      try {
        ...
        const mapped = data.map(
          (p: {
            id: string;
            title: string;
            subject?: string | { id?: string; name?: string; slug?: string };
            difficulty?: string;
            fileUrl?: string;
            pdfUrl?: string;
          }) => ({
            ...
            title: p.title,
            subjectId:
              typeof p.subject === "object" && p.subject?.id
                ? p.subject.id
                : undefined,
            subject:
        ...
        const unlocksFromEntitlement =
          typeof res.data?.entitlement?.freePaperUnlocks === "number"
            ? res.data.entitlement.freePaperUnlocks
            : 0;
        if (!isRolePremium) {
          setFreePaperUnlocks(unlocksFromEntitlement);
        }
    ...
    void loadPapers();
  }, [user, hasPaperAccess, isRolePremium, filter]);
  ...
  const markDone = async (paperId: string) => {
    await api.patch(`/papers/${paperId}/complete`);
    ...
    return (
      <div className="page-container min-h-[80vh] flex flex-col items-center justify-center pt-40 md:pt-52">
        ...
        <p className="text-white/40 text-lg font-medium mb-12 max-w-md text-center leading-relaxed">
          The Academic Vault is encrypted. Please authenticate to access our
          exclusive practice parameters.
        </p>
        <Link to="/login">
  ...
  if (!hasPaperAccess) {
    const xpToNextUnlock =
      quizSummary && typeof quizSummary.nextUnlockXp === "number"
        ? Math.max(0, quizSummary.nextUnlockXp - quizSummary.totalXp)
        : null;
    ...
    return (
      <div className="page-container min-h-[80vh] flex flex-col items-center justify-center pt-40 md:pt-52">
        ...
        <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter mb-4 text-center">
          Elite Protocol <span className="text-gradient">Required.</span>
        </h1>
        <p className="text-white/40 text-lg font-medium mb-16 max-w-xl text-center leading-relaxed">
          Earn quiz XP to unlock free premium papers, or upgrade your membership
          for full unlimited access.
        </p>

        {quizSummary && (
          <div className="mb-10 rounded-2xl border border-amber-400/30 bg-amber-500/10 p-5 text-center max-w-xl">
            <p className="text-[10px] uppercase tracking-[0.2em] text-amber-200/70 mb-2 font-black">
              Quiz Progress Wallet
            </p>
            <p className="text-white font-bold text-sm">
              Badge: {quizSummary.currentBadge} | Total XP:{" "}
              {quizSummary.totalXp}
            </p>
            <p className="text-amber-100/90 text-sm mt-2">
              Free Papers Unlocked: {quizSummary.freePaperUnlocks}
              {typeof xpToNextUnlock === "number"
                ? ` | XP needed for next free paper: ${xpToNextUnlock}`
                : " | All free unlock tiers completed"}
            </p>
          </div>
        )}
      ...
      >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 pt-8">
          <p className="text-white/40 text-lg font-medium max-w-xl leading-relaxed">
            Central repository for past examinations and technical practice
            modules. Engineered for definitive performance scaling.
          </p>
          ...
          <div className="flex flex-wrap gap-4">
            <div className="relative group">
              <select
                ...
                title="Filter papers by subject"
            ...
            <div className="relative group">
              <select
                ...
                aria-label="Filter papers by difficulty"
                title="Filter papers by difficulty"
                className="pl-8 pr-12 py-4 bg-white/[0.02] border border-white/10 rounded-2xl text-white appearance-none focus:bg-white focus:text-black transition-all text-[10px] font-black uppercase tracking-widest min-w-[180px]"
              >
                <option value="" className="bg-black text-white">
                  Difficulties
                        ...
                        <div
                          className={cn(
                            ...
                            isDone
                              ...
                              : "bg-white/5 text-white/20 group-hover:bg-purple-500/20 group-hover:text-purple-300",
                          )}
                        >
                          {isDone ? (
      ...
      {!loading && papers.length === 0 && (
        <div className="flex flex-col items-center justify-center p-32 border border-dashed border-white/5 rounded-[4rem] bg-white/[0.01]">
          ...
          <p className="text-white/20 text-[10px] font-black uppercase tracking-[0.3em]">
            No paper nodes identified for current query.
          </p>
        </div>
      )}
    </div>
